...
3. Generate a Fernet key for Airflow using the command `docker run --rm python:3.11-slim /bin/bash -c \
...
4. Insert the command `docker compose up -d` with the terminal open on the project root folder (or build the containers using your preferred tool, like the VS Code extension)
